...
def make_players_table(data, cur, conn):
  ...
  for person in data["squad"]:
      ...
      #find position id based on position
      pos_id = cur.execute('SELECT id FROM Positions WHERE position = ?', (position,))
      pos_id = cur.fetchone()[0]
      birthyear = int(person.get("dateOfBirth")[0:4])
      nation = person.get("nationality", None)
      #insert data into table 
      cur.execute('INSERT OR IGNORE INTO Players VALUES (?,?,?,?,?)', (id, name, pos_id, birthyear, nation))
      conn.commit()
